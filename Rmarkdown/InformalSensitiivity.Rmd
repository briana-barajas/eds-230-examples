...
# read in data for function (R formated)
load("../Data/sierraczosolar.rda")

# already in the format required for the model (View data)
head(sierraczosolar)

# run the model
solarpv(area=0.1, solar=sierraczosolar, clr="seagreen", eunit="W")

# run and save results - but don't plot
site1 <- solarpv(area=0.1, solar=sierraczosolar, clr="seagreen", eunit="W", g=FALSE)
site1$mean
site1$annual

# consider a different pv array that only uses 
# diffuse and has non standard efficiency (0.6)
site2 <- solarpv(area=0.1, solar=sierraczosolar, clr="yellowgreen", eunit="W", g=FALSE, eff=0.6, etype="direct")
site2$mean
...
# review documentation, .x represents a list or atomic vector
eff <- rnorm(mean=0.6, sd = 0.1, n=20)
site2 <- eff %>% map(~solarpv( area=0.1, solar=sierraczosolar, clr="maroon", eunit="W", g=FALSE, etype="direct", eff=.x ))
